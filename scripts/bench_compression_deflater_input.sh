...
SRC="$ROOT/bench/network-compression/DeflaterByteBufferInputBench.java"
OUT="$ROOT/bench/network-compression/.classes"
REPORT="$ROOT/reports/compression-deflater-input-bench.txt"
...

{
  ...
  echo "command=java -cp \"$OUT:\$(cat $RUNTIME_CP_FILE)\" DeflaterByteBufferInputBench"
  java -cp "$OUT:$RUNTIME_CP" DeflaterByteBufferInputBench
} | tee "$REPORT"

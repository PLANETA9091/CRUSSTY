...
SRC="${ROOT}/bench/ticket-expire/TicketExpireBench.java"
...
OUT="${ROOT}/bench/ticket-expire/.classes"
REPORT="${ROOT}/reports/ticket-expire-bench.txt"
...

{
  ...
  echo "command=java -cp \"${OUT}:\$(cat ${RUNTIME_CP_FILE})\" TicketExpireBench"
  java -cp "${OUT}:${RUNTIME_CP}" TicketExpireBench
} | tee "${REPORT}"

...
OUT="$ROOT/bench/native-reference-list/.classes-bridge"
REPORT="$ROOT/reports/native-reference-list-bridge-bench.txt"
...
javac -cp "$RUNTIME_CP" -d "$OUT" \
  ...
  "$ROOT/bench/native-reference-list/NativeReferenceListBridgeBench.java"

{
  ...
  echo "command=java ${JAVA_ARGS[*]} -Djava.library.path=\"$LIB_DIR\" -cp \"$OUT:$RUNTIME_CP\" NativeReferenceListBridgeBench"
  java "${JAVA_ARGS[@]}" -Djava.library.path="$LIB_DIR" -cp "$OUT:$RUNTIME_CP" NativeReferenceListBridgeBench
} | tee "$REPORT"

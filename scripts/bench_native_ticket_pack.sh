...
OUT="$ROOT/bench/native-ticket-pack/.classes"
REPORT="$ROOT/reports/native-ticket-pack-bench.txt"
...
javac -cp "$RUNTIME_CP" -d "$OUT" \
  "$ROOT/bench/native-ticket-pack/PaperNativeTicketPack.java" \
  "$ROOT/bench/native-ticket-pack/NativeTicketPackBench.java"

{
  ...
  echo "command=java ${JAVA_ARGS[*]} -Djava.library.path=\"$LIB_DIR\" -cp \"$OUT:$RUNTIME_CP\" NativeTicketPackBench"
  java "${JAVA_ARGS[@]}" -Djava.library.path="$LIB_DIR" -cp "$OUT:$RUNTIME_CP" NativeTicketPackBench
} | tee "$REPORT"

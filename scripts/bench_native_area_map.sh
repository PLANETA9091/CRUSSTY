...
OUT="$ROOT/bench/native-area-map/.classes"
REPORT="$ROOT/reports/native-area-map-bench.txt"
...
javac -cp "$RUNTIME_CP" -d "$OUT" \
  "$ROOT/bench/native-area-map/PaperNativeAreaMap.java" \
  "$ROOT/bench/native-area-map/NativeAreaMapBench.java"

{
  ...
  echo "command=java ${JAVA_ARGS[*]} -Djava.library.path=\"$LIB_DIR\" -cp \"$OUT:$RUNTIME_CP\" NativeAreaMapBench"
  java "${JAVA_ARGS[@]}" -Djava.library.path="$LIB_DIR" -cp "$OUT:$RUNTIME_CP" NativeAreaMapBench
} | tee "$REPORT"

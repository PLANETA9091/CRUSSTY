...
OUT="$ROOT/bench/native-ticket-compare/.classes"
REPORT="$ROOT/reports/native-ticket-compare-bench.txt"
...
javac -cp "$RUNTIME_CP" -d "$OUT" \
  "$ROOT/bench/native-ticket-compare/PaperNativeTicketCompare.java" \
  "$ROOT/bench/native-ticket-compare/NativeTicketCompareBench.java"

{
  ...
  echo "command=java ${JAVA_ARGS[*]} -Djava.library.path=\"$LIB_DIR\" -cp \"$OUT:$RUNTIME_CP\" NativeTicketCompareBench"
  java "${JAVA_ARGS[@]}" -Djava.library.path="$LIB_DIR" -cp "$OUT:$RUNTIME_CP" NativeTicketCompareBench
} | tee "$REPORT"

...
OUT="$ROOT/bench/native-chunk-ticket-stage/.classes"
REPORT="$ROOT/reports/native-chunk-ticket-stage-bench.txt"
...
javac -cp "$RUNTIME_CP" -d "$OUT" \
  "$ROOT/bench/native-chunk-ticket-stage/PaperNativeChunkTicketStage.java" \
  "$ROOT/bench/native-chunk-ticket-stage/NativeChunkTicketStageBench.java"

{
  ...
  echo "command=java ${JAVA_ARGS[*]} -Djava.library.path=\"$LIB_DIR\" -cp \"$OUT:$RUNTIME_CP\" NativeChunkTicketStageBench"
  java "${JAVA_ARGS[@]}" -Djava.library.path="$LIB_DIR" -cp "$OUT:$RUNTIME_CP" NativeChunkTicketStageBench
} | tee "$REPORT"

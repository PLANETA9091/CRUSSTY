...
OUT="$ROOT/bench/native-reference-list/.classes"
REPORT="$ROOT/reports/native-reference-list-bench.txt"
...
javac -cp "$RUNTIME_CP" -d "$OUT" \
  ...
  "$ROOT/bench/native-reference-list/NativeReferenceListBench.java"

{
  ...
  echo "command=java ${JAVA_ARGS[*]} -Djava.library.path=\"$LIB_DIR\" -cp \"$OUT:$RUNTIME_CP\" NativeReferenceListBench"
  java "${JAVA_ARGS[@]}" -Djava.library.path="$LIB_DIR" -cp "$OUT:$RUNTIME_CP" NativeReferenceListBench
} | tee "$REPORT"

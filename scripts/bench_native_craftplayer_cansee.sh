...
OUT="${ROOT}/bench/native-craftplayer-cansee/.classes"
REPORT="${ROOT}/reports/craftplayer-cansee-native-bench.txt"
...
javac -proc:none -cp "${FASTUTIL_JAR}" -d "${OUT}" \
  "${ROOT}/bench/native-craftplayer-cansee/PaperNativeCraftPlayerCanSee.java" \
  "${ROOT}/bench/native-craftplayer-cansee/NativeCraftPlayerCanSeeBench.java"

{
  ...
  echo "native_lib=${LIB}"
  echo "command=java -Xms512m -Xmx512m -Djava.library.path=\"${LIB_DIR}\" -cp \"${OUT}:${FASTUTIL_JAR}\" NativeCraftPlayerCanSeeBench"
  java -Xms512m -Xmx512m -Djava.library.path="${LIB_DIR}" -cp "${OUT}:${FASTUTIL_JAR}" NativeCraftPlayerCanSeeBench
} | tee "${REPORT}"

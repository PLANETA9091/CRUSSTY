...
OUT="${ROOT}/bench/nbt-compound-map/.classes"
mkdir -p "${OUT}" "${ROOT}/reports"

javac -proc:none -cp "${RUNTIME_CP}" -d "${OUT}" "${ROOT}/bench/nbt-compound-map/NbtCompoundMapCapacityBench.java"

{
  ...
  printf '\n'
  java -Xms1g -Xmx1g \
    -Dmax.chunks="${NBT_CHUNK_SAMPLE_LIMIT:-256}" \
    -cp "${OUT}:${RUNTIME_CP}" \
    NbtCompoundMapCapacityBench "${samples[@]}"
} | tee "${ROOT}/reports/nbt-compound-map-capacity-bench.txt"

...
## Verified examples (`modules/examples-multilang/`)

Each of `c`, `cpp`, `python`, `js` ships a `build.sh` that links against
`cplug-abi/cplug-abi.h`, a `cplugin.json`, and a module body. All four were
exercised end-to-end on a live Purpur 1.21.10 server: the runtime loads the
four `.so`s, and every class load fires all four hooks (16,000+ hook
